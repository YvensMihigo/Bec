...
export const getAddressesUser = async (req, res = response ) => {
    ...
    try {
        ...
        const addressesdb = await pool.query('SELECT a.id, CONCAT(a.numero_maison, ",", a.avenue, ",",a.commune,",",a.province,",",a.pays) as pays, a.reference, a.Latitude, a.Longitude, p.phone as phone FROM addresses as a INNER JOIN Person as p ON a.persona_id = p.uid WHERE a.persona_id = ?', [req.uid]);
...
export const addStreetAddress = async ( req, res = response ) => {
    ...
    try {
        ...
        const { pays, province, commune, avenue, numero_maison, instruction_speciale, reference, latitude, longitude } = req.body;


        await pool.query('INSERT INTO addresses (pays, province, commune, avenue, numero_maison, instruction_speciale, reference, Latitude, Longitude, persona_id) VALUE (?,?,?,?,?,?,?,?,?,?)', [ pays, province, commune, avenue, numero_maison, instruction_speciale, reference, latitude, longitude, req.uid ]);
